...
#greyout some rows 
# Define the range of cells to grey out (e.g., A1 to C3)
...
query = "SELECT * FROM developer.loanbalancing_xxx"
...
#sheetname['A23'].fill  = grey_fill

grey_range = sheetname['A23:H25']
#loop thru 
...
#Grey out shadow
shadow_grey_range = sheetname['A51:H53']
#loop thru 
#for row in shadow_grey_range:
#    for show_cell in row:
#        show_cell.fill = grey_fill 
...
#sheetname['A62'] = "1) Principal Balance" 
#sheetname['A63'] = "   Loans with balances not converting"
#sheetname['G63'] = "Net Variance" 
#sheetname['G63'].font  = color_font

#Border the excel to medium 
for row in sheetname['A61:H61']:
    for expl_top_cell in row:
        expl_top_cell.border = top_border 
        expl_top_cell.fill = grey_fill 
...
cursor = conn.cursor()
# Write and execute the SELECT query
query = "SELECT BALANCETYPE ,LAONNOTCONAMT,JUSTIFYCONDITION,JUSTIFYCONDITIONAMT,JUSTIFYCONDITION1,JUSTIFYCONDITIONAMT1,JUSTIFYCONDITION2, JUSTIFYCONDITIONAMT2 FROM developer.loanbalancing_xxx X where variance >  0 "
cursor.execute(query)
...
cursor = conn.cursor()
# Write and execute the SELECT query
query = "SELECT * FROM developer.loanbalancing_xxx X "
cursor.execute(query)
...
#select a worksheet
sheettab = wb[" 1)  Principal Balance"] # Replace 'Sheet1' with your desired sheet name

# Write column headers to the Excel sheet
for col_num, column_title in enumerate(columns, start=1):
    sheettab.cell(row=1, column=col_num, value=column_title)


# Write data rows to the Excel sheet
for row_num, row_data in enumerate(results, start=2):
    for col_num, cell_value in enumerate(row_data, start=1):
        sheettab.cell(row=row_num, column=col_num, value=cell_value)
            
print("Principal loan extraction of exception completed ..")         
